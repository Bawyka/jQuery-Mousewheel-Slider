...
$(window).resize(function(){
   ...
   $(window).bind('mousewheel',function(event,delta){
    ...
    function _slide(){ var dir = delta<0?'-':'+'; $slider.animate({top: dir+'='+_h},1000,ease,function(){ _scroll=true; }); }
    ...
    if (current<0){ ++current; start = false; } 

    if (current>=_length){ --current;  start = false; } 

    

    if (start){ $('#content').effect('highlight','slow');  _slide(); _scroll=false; }
